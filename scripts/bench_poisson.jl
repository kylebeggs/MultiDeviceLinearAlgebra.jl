...
for ndev in 1:ngpus
    ...
    t_w = @elapsed begin
        mdla_solve(A_md, b_md; atol = 1.0e-12, rtol = 1.0e-12)
        sync_all_devices(ndev)
    end
    @printf("%.3f s\n", t_w)
    ...
    for run in 1:nruns
        sync_all_devices(ndev)
        t = @elapsed begin
            _, last_stats = mdla_solve(A_md, b_md; atol = 1.0e-12, rtol = 1.0e-12)
            sync_all_devices(ndev)
        end
        push!(times, t)
    end
    ...
    @printf(
        "  Solve: %.4f s  (%d iters)  rel_residual=%.1e  converged=%s\n",
