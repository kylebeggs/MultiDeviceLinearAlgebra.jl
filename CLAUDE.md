...
- Matrix column indices are **remapped to local numbering** at construction (`1:n_owned` owned, then ghosts); `src/gather.jl` reverses this on the way back to the host, `_localize_colval!` does the same on device. Raw device column indices are not global indices.
- **Rows and columns have separate partitions.** `MultiDeviceSparseMatrixCSR` carries both `row_spec` and `col_spec`, so rectangular operators (a prolongation `P`, a restriction `R`) are representable. Column *ownership* — and therefore the ghost set, the `GhostExchange`, and the `x` argument of `mul!` — is resolved against `col_spec`, never `row_spec`. They are the same object for a square matrix built from one spec, which is why `col_spec === row_spec` is a meaningful test. Anything that reaches for `row_spec` to interpret a column is wrong and will produce plausible numbers rather than an error.
- **Device index arrays are always `Int32`** (`DeviceIndex`), regardless of the host `SparseMatrixCSC`'s index type, because cuSPARSE's sparse-sparse routines — SpGEMM, `csr2csc`, `geam` — accept only 32-bit indices. SpMV is the exception that let 64-bit indices work here for as long as it was the only operation. `gather` returns host-native `Int`.
- `PartitionSpec` defines no `==`, so the fallback compares field *identity* and two specs describing the same partition are never equal. Compare with `_specs_match`.
- No submodules — adding an `include` in the wrong position breaks the load order.
- GPU tests are gated on `CUDA.functional()` and sweep `DEVICE_COUNTS = unique(filter(<=(NGPUS), [1, 2, 4, NGPUS]))`. The self-hosted runner is granted 2 devices (`--need 2 --prefer-far`), so in CI that is `[1, 2]` — anything that must hold at 4+ has to be provable by a CPU-tier test or run by hand. The CPU-only tests (`test/test_partition.jl`, `test/test_ghost.jl`, `test/test_poisson.jl`) always run, so a green suite on a CPU box proves very little.
