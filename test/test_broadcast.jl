@testset "MultiDeviceVector broadcasting" begin
    ...
    @testset "y .= α .* x" begin
        ...
        y_md = similar(x_md)
        y_md .= α .* x_md
        @test gather(y_md) ≈ α .* x_cpu rtol = 1.0e-12
    end
    ...
    @testset "y .= x .+ z" begin
        ...
        y_md = similar(x_md)
        y_md .= x_md .+ z_md
        @test gather(y_md) ≈ x_cpu .+ z_cpu rtol = 1.0e-12
    end
    ...
    @testset "y .= α .* x .+ β .* z" begin
        ...
        y_md = similar(x_md)
        y_md .= α .* x_md .+ β .* z_md
        @test gather(y_md) ≈ α .* x_cpu .+ β .* z_cpu rtol = 1.0e-12
    end
    ...
    @testset "In-place y .= y .+ x" begin
        ...
        y_md .= y_md .+ x_md
        @test gather(y_md) ≈ y_cpu .+ x_cpu rtol = 1.0e-12
    end
    ...
    @testset "y .= x ./ s (scalar division)" begin
        ...
        y_md = similar(x_md)
        y_md .= x_md ./ s
        @test gather(y_md) ≈ x_cpu ./ s rtol = 1.0e-12
    end
end
